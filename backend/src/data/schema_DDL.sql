...
DROP TRIGGER IF EXISTS transcribe_insert;
DROP TRIGGER IF EXISTS transcribe_delete;
DROP TRIGGER IF EXISTS transcribe_update;
...
CREATE TRIGGER transcribe_delete AFTER DELETE ON transcriptions FOR EACH ROW
BEGIN
    DELETE FROM transcriptions_idx WHERE created_on = OLD.created_on;
END

CREATE TRIGGER transcribe_update AFTER UPDATE ON transcriptions FOR EACH ROW
BEGIN
    DELETE FROM transcriptions_idx WHERE created_on = OLD.created_on;
  INSERT INTO transcriptions_idx(rowid, created_on, audio_file_name, transcription)
 VALUES (new.id, new.created_on, new.audio_file_name, new.transcription);
